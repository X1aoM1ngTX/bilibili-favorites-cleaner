...
const loadFolders = async () => {
  ...
  try {
    const response = await fetch('/api/sort/folders')
    const result = await response.json()

    if (result.success) {
      folders.value = [...result.data]
      sortedFolders.value = [...result.data]
    } else {
      error.value = result.error || '获取收藏夹列表失败'
    }
  } catch (err) {
...
const saveSort = async () => {
  ...
  try {
    ...
    const response = await fetch('/api/sort/execute', {
      ...
      body: JSON.stringify({ folderIds: finalSortIds })
    })
    ...
    if (result.success) {
      ...
      const errorMessage = result.error || '排序失败'
      sortResult.value = {
        success: false,
